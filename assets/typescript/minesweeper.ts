...
interface Square {
    box: HTMLTableDataCellElement,
    hasMine: boolean,
    minedNeighbors: number,
    isRevealed: boolean,
    isFlagged: boolean,
}
...
function getMinedCoords(): number[] {
...
function clickMineSquare(event: MouseEvent) {
    if (gameOver) {
        return;
    }
...
function rightClickMineSquare(event: MouseEvent) {
    if (gameOver) {
        return;
    }
    let element = event.target as any;
...
function clickMineCoords(x: number, y: number, forceReveal: boolean = false) {
